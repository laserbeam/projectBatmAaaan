/**
 * This is an implementation of The McLeod Pitch Method (MPM) for pitch detection as described at http://miracle.otago.ac.nz/postgrads/tartini/papers/A_Smarter_Way_to_Find_Pitch.pdf.
 * This is based on a java implementation retrieved from https://raw.github.com/JorenSix/TarsosDSP/master/src/be/hogent/tarsos/dsp/pitch/McLeodPitchMethod.java on 20 Mar 2013.
 *
 * CHECK THE WEBSITES ABOVE FOR DETAILS. (if you care)
 */


// The expected size of an audio buffer (in samples).
final int MPM_DEFAULT_BUFFER_SIZE = 1024;
...
final int MPM_DEFAULT_OVERLAP = 768;
...
class McLeodPitchMethod {
  ...
  private final ArrayList<Integer> maxPositions = new ArrayList<Integer>();

  // A list of estimates of the period of the signal (in samples).
  private final ArrayList<Float> periodEstimates = new ArrayList<Float>();

  // A list of estimates of the amplitudes corresponding with the period estimates.
  private final ArrayList<Float> ampEstimates = new ArrayList<Float>();
  ...
  public McLeodPitchMethod(final float audioSampleRate) {
    this(audioSampleRate, MPM_DEFAULT_BUFFER_SIZE, MPM_DEFAULT_CUTOFF);
  }
  ...
  public McLeodPitchMethod(final float audioSampleRate, final int audioBufferSize) {
    this(audioSampleRate, audioBufferSize, MPM_DEFAULT_CUTOFF);
  }
...
// A result returned by the pitch detector
